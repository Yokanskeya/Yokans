...
module.exports = class Loop extends Interaction {
    ...
    async exec(int, data) {
        ...
        if (int.guild.members.me.voice.channel && channel !== int.guild.members.me.voice.channel)
            return int.reply({
                content: `❗️ You must be in the same voice channel as me to use this command!`,
                ephemeral: true,
            });
        ...
        if (data.voiceChannels.length > 0 && !isAllowed) {
            return int.reply({
                content: `❗️ You must be in one of the allowed voice channels to use this command!`,
                ephemeral: true,
            });
        }
        ...
        if (members.size > 1 && !isDJ && !int.member.permissions.has("MANAGE_GUILD")) {
            return int.reply({
                content:
                    "❗️ You must be a DJ or be alone in the voice channel to use this command!",
                ephemeral: true,
            });
        }
        ...
        if (!hasQueue) {
            return int.reply({
                content: "❗️ There is no music playing in this guild!",
                ephemeral: true,
            });
        }
        ...
        if (mode === "track") {
            if (queue.repeatMode === RepeatMode.SONG) {
                ...
                return int.reply({
                    content: `🔂 Looped the current track!`,
                    ephemeral: true,
                });
            }
        } else if (mode === "queue") {
            if (queue.repeatMode === RepeatMode.QUEUE) {
                ...
                return int.reply({
                    content: `🔁 Looped the current queue!`,
                    ephemeral: true,
                });
            }
            ...
            if (queue.repeatMode === RepeatMode.DISABLED) {
                return int.reply({
                    content: "❗️ The loop mode is already disabled!",
                    ephemeral: true,
                });
            } else {
                queue.setRepeatMode(RepeatMode.DISABLED);
                return int.reply({
                    content: `❗️ Disabled the loop mode!`,
                    ephemeral: true,
                });
            }
        }
