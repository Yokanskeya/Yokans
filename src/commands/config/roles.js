module.exports = class Roles extends Interaction {
    ...
    async exec(int, data) {
        if (!int.member.permissions.has("MANAGE_GUILD"))
            return int.reply({
                content: "❗️ You don't have the required permissions to do this!",
                ephemeral: true,
            });
        ...
        if (cmd === "remove") {
            ...
            if (!old)
                return int.reply({
                    content: `❗️ The role ${role.name} is not in the list!`,
                    ephemeral: true,
                });
            ...
            return int.reply({
                content: `❗️ Removed role ${role.name} from the DJ roles list!`,
                ephemeral: true,
            });
        }
        if (cmd === "list") {
            ...
            if (!djs.length)
                return int.reply({
                    content: "❗️ There are no DJ roles set!",
                    ephemeral: true,
                });
